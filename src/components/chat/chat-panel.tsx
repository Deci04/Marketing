...
export function ChatPanel({ userName }: { userName: string }) {
  ...
  // Attribution (author name) per persisted message id; live msgs fall back to
  // the current user / "Assistente".
  ...
  const { sendMessage, messages, setMessages, status, addToolApprovalResponse } =
    useChat({
      transport: new DefaultChatTransport({
        api: "/api/chat",
        // The client owns the shared thread state (seeded from the DB), so the
        // full message list is sent — this preserves in-flight tool-approval
        // parts across the confirm round-trip.
        prepareSendMessagesRequest({ messages }) {
          return { body: { messages } };
        },
      }),
      // After the user confirms/denies a proposed action, automatically continue
      // the conversation so the tool runs (or the denial reaches the model).
  ...
  // Load + seed the shared DB history the first time the panel opens, so model
  // context is consistent across users and reloads. Runs once; the seeded
  // messages then become the live `useChat` state (incl. approval round-trips).
  useEffect(() => {
  ...
  // Is there a proposed action waiting on the user? (blocks the composer)
          ...
          {loaded && messages.length === 0 && (
            <div className="mt-10 text-center text-sm text-ink/55">
              ...
              <p>
                ...
                KPI — oppure di creare un blocco, un contenuto o una
                pubblicazione. Confermerai ogni azione prima che venga eseguita.
                La conversazione è visibile a tutto il workspace.
              </p>
            </div>
          )}
